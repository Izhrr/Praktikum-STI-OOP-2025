...
public class BowgunHunter extends Hunter implements WeaponMaster {
    ...
    public void reload() {
        ammoCount = MAX_AMMO;
        setAttackPower(getAttackPower() - RELOAD_PENALTY);
        if(getAttackPower() < 1){
            setAttackPower(1);
        }
    }
    ...
    public void specialAttack(Monster monster) {
        if (super.isStunned()) {
            System.out.println(super.getName() + " is stunned and cannot attack!");
        }
        else{
            if (ammoCount > 0) {
                ammoCount--;
                int damage = (int) (super.getAttackPower() * 1.5);
                ...
                System.out.println(super.getName() + " is out of ammo!");
                reload();
            }
        }
    }
