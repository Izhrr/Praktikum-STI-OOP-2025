/*
    ...
    Deskripsi Program       : Implementasi Agrobacterium
*/
...
public class Agrobacterium extends Bacteria implements AdaptiveResponse, ToxicAttack {
    ...
    public void toxicAttack(BacteriaBehaviour target) {
        target.setCurrentPopulation(target.getCurrentPopulation() - toxinLevel);
        if(target.getCurrentPopulation() < 0){
            target.setCurrentPopulation(0);
        }
    }
    ...
    public void doAdaptiveResponse() {
        setGrowthRate(getGrowthRate()*1.3);
    }
