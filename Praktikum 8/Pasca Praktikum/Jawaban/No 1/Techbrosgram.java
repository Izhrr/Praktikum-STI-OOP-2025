...
public class Techbrosgram {
    ...
    public void registerBros(String name) {
        // Menggunakan putIfAbsent untuk memastikan tidak menimpa Bro yang sudah ada.
    ...
    public void likePost(String likerBroName, int postId) {
        Bro liker = bros.get(likerBroName);
        Post post = posts.get(postId);

        // Memastikan liker dan post valid, dan liker bukan pemilik post.
        if (liker != null && post != null && !post.author.name.equals(likerBroName)) {
    ...
    public void displayPopularPosts() {
        ...
        for (Post post : sorted) {
            System.out.printf("[%d] %s oleh %s - Likes: %d\n",
                    post.id, post.content, post.author.name, post.likes.size());
        }
    }
