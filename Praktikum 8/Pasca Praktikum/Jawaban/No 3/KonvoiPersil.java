...
public class KonvoiPersil {
    ...
    public int hitungTotalPendukungKonvoi() {
        Set<Pendukung> pendukungUnik = new HashSet<>();
        for (Kendaraan kendaraan : daftarGrup.keySet()) {
            pendukungUnik.addAll(kendaraan.getDaftarPendukung());
        }
        return pendukungUnik.size();
    }
    ...
    public Pendukung cariPendukungTertuaDalamKendaraan(Kendaraan kendaraan) {
        ...
        Pendukung tertua = null;
        int umurTertua = -1;
        
        for (Pendukung pendukung : kendaraan.getDaftarPendukung()) {
            if (pendukung.getUmur() > umurTertua) {
                umurTertua = pendukung.getUmur();
                tertua = pendukung;
            }
        }
