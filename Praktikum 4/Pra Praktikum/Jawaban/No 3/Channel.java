...
public class Channel implements Poster {
    ...
    public void post(Content video) {
        if(videoCount < maxVideos && video instanceof Video){
            ...
            for(Subscriber subscriber : subscribers){
                if(subscriber != null){
                    subscriber.addNotification("Channel " + channelName + " telah mengunggah video baru dengan judul " + video.getTitle());
                }
            }
        }
    }
    ...
    public void changePost(Content oldVideo, Content newVideo) {
        if(oldVideo instanceof Video && newVideo instanceof Video){
            Video oldVid = (Video) oldVideo;
            Video newVid = (Video) newVideo;
            for(int i = 0 ; i < maxVideos ; i++){
                if(videos[i].isSame(oldVid)){
                    videos[i] = newVid;

                    for(Subscriber subscriber : subscribers){
                        if(subscriber != null){
                            subscriber.addNotification("Channel " + channelName + " telah mengubah video " + oldVid.getTitle() + " menjadi " + newVid.getTitle());
                        }
                    }
                    return;
                }
