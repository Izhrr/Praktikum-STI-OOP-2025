...
public class InventoryManager<A extends Barang> {
    ...
    public void addCatalog(A item, int jumlah) {
        inventory.add(new CatalogBarang<A>(item, jumlah));
    }
    ...
    public void printTotalType() {
        ...
        for(int i = 0; i < inventory.size(); i ++){
            if(inventory.get(i).barang() instanceof Baju){
                totalBaju++;
            }
            if(inventory.get(i).barang() instanceof Handphone){
                totalHandphone++;
            }
        }
    ...
    public int getTotalAmount(String itemType) {
        ...
        int amount = 0;
        for (int i = 0; i < inventory.size(); i++) {
            if (inventory.get(i).barang().getClass().getSimpleName().equals(itemType)) {
                amount += inventory.get(i).jumlah();
            }
        }
        return amount;
    }
    ...
    public int getTotalPrice() {
        int totalPrice = 0;
        for(int i = 0; i < inventory.size() ; i++){
            totalPrice += inventory.get(i).barang().getPrice() * inventory.get(i).jumlah();
        }
        return totalPrice;
    }
    ...
    public void printInventory() {
        if(inventory.isEmpty()){
            ...
            for(int i = 0; i < inventory.size() ; i ++){
                String namaProduk = inventory.get(i).barang().getName();
                int hargaProduk = inventory.get(i).barang().getPrice();
                int jumlahProduk = inventory.get(i).jumlah();
